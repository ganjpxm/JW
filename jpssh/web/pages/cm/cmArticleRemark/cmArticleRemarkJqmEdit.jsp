...
<div data-role="dialog">
  <div id="header" data-role="header" data-theme="d">
	<h1><fmt:message key="cmArticleRemark.edit.title"/></h1>
  </div>
  <div id="content" data-role="content" data-theme="c">
	<form id="cmArticleRemarkForm" action="<c:url value='/updateCmArticleRemark'/>" method="post" data-ajax="false">
	  <input type="hidden" name="articleRemarkId" value="${cmArticleRemark.articleRemarkId}"/>
	  <input type="hidden" name="menuId" value="${menuId}"/>
	  <!--articleRemarkId,articleId,remark,displayNo,operatorId,operatorName,createDateTime,modifyTimestamp,dataState,-->
	  <div class="lable"><span class="red">*</span><fmt:message key="cmArticleRemark.fieldName1"/> : </div>
      <input id="fieldName1" name="fieldName1" class="validate[required]" value="${cmArticleRemark.fieldName1}" class="validate[required]"/>
...
$(document).ready(function(){
  $("#cmArticleRemarkForm").validationEngine();
});

function save() {
  if (!$('#cmArticleRemarkForm').validationEngine('validate')) {
	return false;
  }
  $("#cmArticleRemarkForm").submit();
