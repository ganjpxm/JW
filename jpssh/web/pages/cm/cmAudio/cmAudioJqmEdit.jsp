...
<div style="margin: 2px 20px;">
  <div id="header" data-role="header" data-theme="d">
	<h1><fmt:message key="cmAudio.edit.title"/></h1>
  </div>
  <div id="content" data-role="content" data-theme="c">
	...
	<form id="cmAudioForm" action="<c:url value='/updateCmAudio'/>" method="post" data-ajax="false">
	  <input type="hidden" name="audioId" value="${cmAudio.audioId}"/>
	  <input type="hidden" name="menuId" value="${menuId}"/>
	  <input type="hidden" name="categoryId" value="${categoryId}"/>
	  <input type="hidden" id="newCategoryIds" name="newCategoryIds"/>
	  <input type="hidden" id="roleIds" name="roleIds" value="${cmAudio.roleIds}"/>
      ...
      <input id="title" name="title" value="${cmAudio.title}"/>
      <div class="lable"><fmt:message key="common.field.tag"/> : </div>
      <input id="tag" name="tag" value="${cmAudio.tag}"/>
      
      <div class="lable">Cover Url : 
      <input id="thumbUrl" name="thumbUrl" value="${cmAudio.thumbUrl}"/>
      <c:choose>
		<c:when test="${fn:startsWith(cmAudio.thumbUrl, 'http') || fn:startsWith(cmAudio.thumbUrl, '/')}">
		    <img id="thumbImage" src="<c:url value='${cmAudio.thumbUrl}'/>" height="70"/>
		</c:when>
		<c:otherwise>
		  <c:if test="${cmAudio.thumbUrl!=''}">
		  	  <img id="thumbImage" src="<c:url value='${ImageUrl}/${cmAudio.thumbUrl}'/>" height="70"/>
	      </c:if>
		</c:otherwise>
	  </c:choose>
	  ...
	  <div class="lable"><fmt:message key="common.field.url"/>(audioName1+audioUrl1;) : 
      ...
      <input id="displayNo" name="displayNo" value="${cmAudio.displayNo}"/>
      
      <div class="lable"><fmt:message key="common.field.description"/> : 
      <textarea name="description" cols="40" rows="20" style="height:100px;">${cmAudio.description}</textarea>

      <div class="lable"><fmt:message key="common.field.remark"/> : 
      <textarea name="remark" cols="40" rows="20" style="height:100px;">${cmAudio.remark}</textarea>
