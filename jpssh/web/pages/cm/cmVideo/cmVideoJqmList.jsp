...
<div id="content" data-role="content">
  <div class="jpw-content">
	<div class="ui-bar ui-bar-c">
	  <div class="jqm-list-title"><fmt:message key="cmVideo.list.title"/></div>
	</div>
	<div class="ui-bar ui-bar-c">
	  <form id="cmVideoForm" method="post">
	    <input type="hidden" name="menuId" value="${menuId}"/>
	    <input type="checkbox" id="checkHead" onclick="jp.checkAll(this,'videoId')" style="margin-top:8px;" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  
	      <a href="javascript:toPage('<c:url value='/cmVideoJqmAdd?menuId=$ {menuId}'/>')" data-role="button" data-icon="jp-add" data-mini="true" 
	...
	<div class="withcheckbox">
	  <ul data-role="listview" data-filter="true" data-theme="d" data-split-icon="gear" data-split-theme="d">
		<c:forEach var="model" items="${cmVideos}">
		  <li><input type="checkbox" name="videoId" value="$ {model.videoId}"/>
		    <a href="javascript:popupDetailDialog('$ {model.videoId}','${model.filedName}')">
		      <h3>${model.filedName}</h3><p></p></a>
			  <a href="javascript:popupDialog('$ {model.videoId}','${model.filedName}');" 
...
<!--
function popupDialog(videoId,fieldName) {
  $('<div>').simpledialog2({
    mode: 'button',
    ...
    buttons : {
      '<fmt:message key="button.edit"/>': {
        click: function () { 
        	window.location.href="<c:url value='/cmVideoJqmEdit/'/>" + videoId + "?menuId=$ {menuId}";;
        	showLoading();
        },
        icon: "jp-edit" , theme: "c"
      },
      '<fmt:message key="button.delete"/>': {
        click: function () { confirmDelete(videoId);},
...
function popupDetailDialog(videoId, filedName) {
  $("<div>").simpledialog2({
    mode:'blank',
    width:'300px',
    blankContent: 
    	"<div class='tc m10'><h3>" + "<fmt:message key='cmVideo.detail.title'/>" + "</h3></div>" + 
    	"<div class='p10'><fmt:message key='cmVideo.filedName'/> : <span class='underline'>" + filedName + "</span></div>" +
    	...
    	"<a rel='external' data-role='button' data-mini='true' data-inline='true' data-theme='${jqmTheme}'" + 
    	   "href='javascript:confirmDelete(&quot;" + videoId + "&quot;);' class='fr'><fmt:message key='button.delete'/></a>"+   
    	"<a rel='external' data-role='button' data-mini='true' data-inline='true' data-theme='${jqmTheme}'" + 
    	   "href='<c:url value='/cmVideoJqmEdit/'/>" + videoId + "' class='fr'><fmt:message key='button.edit'/></a>"
  });
};

function confirmDelete(videoId) {
  confirm(I18N.msg_del_confirm, function () {
  	window.location.href="<c:url value='/deleteCmVideo/'/>" + videoId + "?menuId=$ {menuId}";
  });
}

function batchDelete() {
  var checkValues = jp.getCheckValues("videoId");
  if (jp.isEmpty(checkValues)) {
	showAlert(I18N.msg_no_sel);
  } else {
	  confirm(I18N.msg_del_confirm, function () {
		  var formEl = document.getElementById("cmVideoForm");
		  if ($("#videoIds").val()) {
			$("#videoIds").val(checkValues);
		  } else {
			formEl.appendChild(jp.createHidden("videoIds", checkValues));
		  }
		  formEl.action = "<c:url value='/deleteCmVideos'/>";
		  formEl.submit();
		  showLoading();
	  });
  }
