  ...
  <div class="jpw-content">
	...
	<div class="ui-bar ui-bar-c">
	  <form id="cmPhotoForm" method="post">
	    ...
	    <input type="checkbox" id="checkHead" onclick="jp.checkAll(this,'photoId')" style="margin-top:8px;" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  
	      <a href="javascript:toPage('<c:url value='/cmPhotoJqmAdd?menuId=$ {menuId}'/>')" data-role="button" data-icon="jp-add" data-mini="true" 
	...
	<div class="withcheckbox">
	  <ul data-role="listview" data-filter="true" data-theme="d" data-split-icon="gear" data-split-theme="d">
		<c:forEach var="model" items="${cmPhotos}">
		  <li><input type="checkbox" name="photoId" value="$ {model.photoId}"/>
		    <a href="javascript:popupDetailDialog('$ {model.photoId}','${model.filedName}')">
		      <h3>${model.filedName}</h3><p></p></a>
			  <a href="javascript:popupDialog('$ {model.photoId}','${model.filedName}');" 
...
<!--
function popupDialog(photoId,fieldName) {
  $('<div>').simpledialog2({
    mode: 'button',
    ...
    buttons : {
      '<fmt:message key="button.edit"/>': {
        click: function () { 
        	window.location.href="<c:url value='/cmPhotoJqmEdit/'/>" + photoId + "?menuId=$ {menuId}";;
        	showLoading();
        },
        icon: "jp-edit" , theme: "c"
      },
      '<fmt:message key="button.delete"/>': {
        click: function () { confirmDelete(photoId);},
...
function popupDetailDialog(photoId, filedName) {
  $("<div>").simpledialog2({
    mode:'blank',
    width:'300px',
    blankContent: 
    	"<div class='tc m10'><h3>" + "<fmt:message key='cmPhoto.detail.title'/>" + "</h3></div>" + 
    	"<div class='p10'><fmt:message key='cmPhoto.filedName'/> : <span class='underline'>" + filedName + "</span></div>" +
    	...
    	"<a rel='external' data-role='button' data-mini='true' data-inline='true' data-theme='${jqmTheme}'" + 
    	   "href='javascript:confirmDelete(&quot;" + photoId + "&quot;);' class='fr'><fmt:message key='button.delete'/></a>"+   
    	"<a rel='external' data-role='button' data-mini='true' data-inline='true' data-theme='${jqmTheme}'" + 
    	   "href='<c:url value='/cmPhotoJqmEdit/'/>" + photoId + "' class='fr'><fmt:message key='button.edit'/></a>"
  });
};
...
function batchDelete() {
  var checkValues = jp.getCheckValues("photoId");
  if (jp.isEmpty(checkValues)) {
	showAlert(I18N.msg_no_sel);
  } else {
	  confirm(I18N.msg_del_confirm, function () {
		  var formEl = document.getElementById("cmPhotoForm");
		  if ($("#photoIds").val()) {
			$("#photoIds").val(checkValues);
		  } else {
			formEl.appendChild(jp.createHidden("photoIds", checkValues));
		  }
		  formEl.action = "<c:url value='/deleteCmPhotos'/>";
		  formEl.submit();
		  showLoading();
	  });
  }
