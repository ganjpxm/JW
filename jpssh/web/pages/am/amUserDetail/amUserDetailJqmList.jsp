...
<div id="content" data-role="content">
  <div class="jpw-content">
	<div class="ui-bar ui-bar-c">
	  <div class="jqm-list-title"><fmt:message key="amUserDetail.list.title"/></div>
	</div>
	<div class="ui-bar ui-bar-c">
	  <form id="amUserDetailForm" method="post">
	    <input type="hidden" name="menuId" value="${menuId}"/>
	    <input type="checkbox" id="checkHead" onclick="jp.checkAll(this,'userDetailId')" style="margin-top:8px;" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  
	      <a href="javascript:toPage('<c:url value='/amUserDetailJqmAdd?menuId=${menuId}'/>')" data-role="button" data-icon="jp-add" data-mini="true" 
	...
	<div class="withcheckbox">
	  <ul data-role="listview" data-filter="true" data-theme="d" data-split-icon="gear" data-split-theme="d">
		<c:forEach var="model" items="${amUserDetails}">
		  <li><input type="checkbox" name="userDetailId" value="${model.userDetailId}"/>
		    <a href="javascript:popupDetailDialog('${model.userDetailId}','${model.filedName}')">
		      <h3>${model.filedName}</h3><p></p></a>
			  <a href="javascript:popupDialog('${model.userDetailId}','${model.filedName}');" 
...
<!--
function popupDialog(userDetailId,fieldName) {
  $('<div>').simpledialog2({
    mode: 'button',
    ...
    buttons : {
      '<fmt:message key="button.edit"/>': {
        click: function () { 
        	window.location.href="<c:url value='/amUserDetailJqmEdit/'/>" + userDetailId + "?menuId=${menuId}";;
        	showLoading();
        },
        icon: "jp-edit" , theme: "c"
      },
      '<fmt:message key="button.delete"/>': {
        click: function () { confirmDelete(userDetailId);},
...
function popupDetailDialog(userDetailId, filedName) {
  $("<div>").simpledialog2({
    mode:'blank',
    width:'300px',
    blankContent: 
    	"<div class='tc m10'><h3>" + "<fmt:message key='amUserDetail.detail.title'/>" + "</h3></div>" + 
    	"<div class='p10'><fmt:message key='amUserDetail.filedName'/> : <span class='underline'>" + filedName + "</span></div>" +
    	...
    	"<a rel='external' data-role='button' data-mini='true' data-inline='true' data-theme='${jqmTheme}'" + 
    	   "href='javascript:confirmDelete(&quot;" + userDetailId + "&quot;);' class='fr'><fmt:message key='button.delete'/></a>"+   
    	"<a rel='external' data-role='button' data-mini='true' data-inline='true' data-theme='${jqmTheme}'" + 
    	   "href='<c:url value='/amUserDetailJqmEdit/'/>" + userDetailId + "' class='fr'><fmt:message key='button.edit'/></a>"
  });
};

function confirmDelete(userDetailId) {
  confirm(I18N.msg_del_confirm, function () {
  	window.location.href="<c:url value='/deleteAmUserDetail/'/>" + userDetailId + "?menuId=${menuId}";
  });
}

function batchDelete() {
  var checkValues = jp.getCheckValues("userDetailId");
  if (jp.isEmpty(checkValues)) {
	showAlert(I18N.msg_no_sel);
  } else {
	  confirm(I18N.msg_del_confirm, function () {
		  var formEl = document.getElementById("amUserDetailForm");
		  if ($("#userDetailIds").val()) {
			$("#userDetailIds").val(checkValues);
		  } else {
			formEl.appendChild(jp.createHidden("userDetailIds", checkValues));
		  }
		  formEl.action = "<c:url value='/deleteAmUserDetails'/>";
		  formEl.submit();
		  showLoading();
	  });
  }
