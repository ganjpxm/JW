...
<div onload="scaleWidth();" id="body">
  <div class="content" style="text-align:center;padding:20px;font-size:20px;min-height:500px;">
    <div id="searchTag" class="item">
      ...
      <button onclick="javascript:searchPhotosByTag();" title="<fmt:message key='button.search'/>" class="searchBtn" style="height:35px;margin-top:-10px;min-width:35px;">&nbsp;</button>
      <div>
        <div id="dycontainerProperty">
  		  ...
  		  <c:if test="${user!=null}">
		      ...
		      <label class="upload_btn fileinput-button">
		        ...
		        <input id="uploadPhoto" type="file" name="file" data-url="<c:url value='/uploadImageToRackspaceCloudFiles'/>" multiple/></form>
		      </label>
		      </div>
  		    </div>
		        ...
		        <div title="${map.tag}" class="box" style="background-color:white;display:block;">
		          <c:choose>
	 				  <c:when test="${fn:startsWith(map.url, 'http') || fn:startsWith(map.url, '/')}">
						<a href="<c:url value='${map.url}'/>">
					  	  <img src="<c:url value='${map.url}'/>" width="100%" style="margin-bottom:0px;"/><br/>
				        </a>
					  </c:when>
	 				  <c:otherwise>
	 				    <c:if test="${map.url!=''}">
	                      <a href="<c:url value='${ImageUrl}/${map.url}'/>">
					  	    <img src="<c:url value='${ImageUrl}/${map.url}'/>" width="100%" style="margin-bottom:0px;"/><br/>
				          </a>
				        </c:if>
					  </c:otherwise>
				  </c:choose>
			      <div style="padding:10px;font-size:16px;font-weight:bold; color:#333;">
			          <!--  ${status.count + pageSize*(pageNo-1)} -->
			          <c:if test="${map.thumbUrl!=''}">
			          	<button title="${map.thumbUrl}" style="background-color:white;border:1px;text-decoration:underline;font-size:16px;font-weight:bold; color:#333; " onclick="javascript:toNewWindow('${map.thumbUrl}');">${map.title}</button>
			          </c:if>
			          <c:if test="${map.thumbUrl==''}">
			            ${map.title}
			          </c:if>
