...
<div id="body">
  <div class="content" style="text-align:center;padding:20px;font-size:20px;min-height:500px;">
    <div id="searchTag" class="item">
      ...
      <button onclick="javascript:searchFilesByTag();" title="<fmt:message key='button.search'/>" class="searchBtn" style="height:35px;margin-top:-10px;">&nbsp;</button>
      <div>
          ...
          <input type="checkbox" id="drag" /><label for="drag"><fmt:message key='common.field.drag'/></label>
  		  ...
  		  <c:if test="${user!=null}">
		      ...
		      <label class="upload_btn fileinput-button">
		        ...
		        <input id="uploadFile" type="file" name="file" data-url="<c:url value='/uploadFile'/>" multiple/></form>
		      </label>
		      </div>
  		    </div>
	    ...
	    <div id="dycontainer" class="dycontainer">
		    <c:forEach var="map" items="${fileMaps}" varStatus="status">
		        <div class="object" style="background-color:white;display:block;">
		          <a href="<c:url value='${map.url}'/>" style="color:#333; text-decoration:none; font-size: 18px; font-family:"MuseoSlabRegular",serif;line-height: 1.27;">
